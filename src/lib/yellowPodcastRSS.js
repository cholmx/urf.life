// Yellow Podcast RSS feed utility
export class YellowPodcastRSSService {
  constructor() {
    this.rssUrl = 'https://anchor.fm/s/eeeaafa8/podcast/rss'
    this.proxies = [
  ...
  parseRSSFeed(xmlString) {
    try {
      ...
      const channelInfo = {
        ...
        description: this.getTextContent(channel, 'description') || 'Yellow Podcast Episodes',
        ...
        author: this.getTextContent(channel, 'itunes\\:author,author') || 'Yellow Podcast'
      }
  ...
  getFallbackData() {
    return {
      channel: {
        ...
        description: 'Welcome to the Yellow Podcast. Join us for engaging conversations and inspiring content.',
        image: 'https://images.unsplash.com/photo-1478737270239-2f02b77fc618?w=400&h=400&fit=crop',
        link: 'https://anchor.fm/s/eeeaafa8/podcast/rss',
        language: 'en',
        author: 'Yellow Podcast'
      },
      episodes: [
        {
          id: 'fallback-1',
          title: 'Welcome to Yellow Podcast',
          description: 'In our inaugural episode, we introduce the Yellow Podcast and share our vision for this journey together.',
          audioUrl: null,
          duration: '25:30',
          pubDate: new Date(Date.now() - 7 * 24 * 60 * 60 * 1000).toISOString(),
          link: 'https://anchor.fm/s/eeeaafa8',
  ...
  getCachedEpisodes() {
    try {
      const cached = localStorage.getItem('yellow_podcast_cache')
      ...
      if (cacheAge > maxAge) {
        localStorage.removeItem('yellow_podcast_cache')
        return null
      }
...
export default new YellowPodcastRSSService()